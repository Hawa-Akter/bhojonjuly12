...
ipcMain.on("orderIdListForMegreOrder", (e, mergerorderIdList) => {
  console.log("mergerorderIdList", mergerorderIdList);
  let mergeOrderInfo = [];
  mergerorderIdList.map((orderId) => {
    ...
    db.all(orderIdsQ, [orderId], (err, rows) => {
      ...
      console.log(rows);
      rows.forEach((row) => {
        mergeOrderInfo.push(rows);
        console.log("merOrderidrows", row);
      });
    });
  ...
  mergeOrderWin.webContents.on("did-finish-load", () => {
    console.log("mergeOrderInfo", mergeOrderInfo);
    mergeOrderWin.webContents.send("orderInfoForMerge", mergeOrderInfo);
    //mergeOrderWin.webContents.send("addOnCartSent", addOns);
  });
});
